/***********
 * smoothShadingB.js
 * based on phongLambertSmoothShading188.js
 * M. Laszlo
 * May 2020
 ***********/
...
let sun;
let matPhong, matLambert, mesh, geom;
let face1, face2;
let normalLines;
...
function createScene() {
    matPhong = new THREE.MeshPhongMaterial({color: 0x0000FF, side: THREE.DoubleSide});
...
function updateNormals() {
    ...
    console.log(face1.vertexNormals)
...
function init() {
    ...
    let shadingTypes =  ['Lambert', 'Phong'];
    let shadetype = gui.add(controls, 'shading', shadingTypes);
    shadetype.onChange(updateShading);
    let shininessControl = gui.add(controls, 'shininess', 0, 200).step(1);
